...
module.exports = function(locals) {
  ...
  for (var i = 0; i < locals.posts.data.length; i++) {
    ...
    if (locals.posts.data[i].push_update == true) {
       locals.posts.data[i].date = locals.posts.data[i].updated;
       locals.posts.data[i].uuid = locals.posts.data[i].uuid +'-'+ locals.posts.data[i].date.toDate().getTime()
    }
  };
  var posts = locals.posts.sort('-date');
  if (feedConfig.limit) posts = posts.limit(feedConfig.limit);
  
  // Hide posts that we don't want the public to see.
  var remove_indexes = []
  posts.forEach(function(post, index){
    if (post.feed_hide == true) {
